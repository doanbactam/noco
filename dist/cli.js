...
async function runInstallCommand() {
  ...
  if (currentName.exists && currentName.value && isAIAuthor(currentName.value)) {
    ...
    logger2.blank();
    logger2.warning(`Current git author: ${source_default.yellow(currentName.value)}`);
    ...
    if (response.name && response.email) {
      ...
      logger2.blank();
      logger2.info("Skipped author update. You can change it later with:");
      logger2.info(`  ${source_default.dim('git config --global user.name "Your Name"')}`);
      logger2.info(`  ${source_default.dim('git config --global user.email "your@email.com"')}`);
      logger2.blank();
    }
  }
