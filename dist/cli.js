...
async function install(options = {}) {
  ...
  try {
    ...
    if (!currentTemplate.exists) {
      ...
      logger2.success(`Git template directory set to ${templateDirForGit}`);
    } else {
      logger2.success("Git template directory already configured");
    }
    return {
      success: true,
...
async function uninstall(options = {}) {
  ...
  try {
    ...
    if (options.removeConfig) {
      logger2.info("Removing git configuration...");
      unsetGitConfig("init.templatedir");
      removedConfig = true;
      logger2.success("Git template directory configuration removed");
    }
    if (!options.removeConfig) {
      logger2.blank();
...
async function runInstallCommand(options) {
  ...
  if (currentName.exists && currentName.value && isAIAuthor(currentName.value)) {
    logger2.header("AI Author Detected");
    logger2.blank();
    logger2.warning(`Current git author: ${logger2.yellowText(currentName.value)}`);
    logger2.info("This looks like an AI-generated name. Would you like to change it?");
    logger2.blank();
    ...
    if (name && email) {
      ...
      logger2.blank();
      logger2.success(`Git author updated to: ${logger2.cyan(name)} <${logger2.cyan(email)}>`);
      logger2.blank();
    } else {
      logger2.blank();
      logger2.info("Skipped author update. You can change it later with:");
      logger2.info(`  ${logger2.dim('git config --global user.name "Your Name"')}`);
      logger2.info(`  ${logger2.dim('git config --global user.email "your@email.com"')}`);
      logger2.blank();
    }
  }
  const current = getTemplateDir();
  if (current.exists && current.value) {
    logger2.info(`Current template: ${logger2.cyan(current.value)}`);
  } else {
  ...
  logger2.blank();
  logger2.info(logger2.dim("For existing repos, run: cd <repo> && git init"));
}
program2.name("nococli").description("Remove AI co-author signatures from git commits").version(version);
...
program2.command("patterns").description("List all AI co-author signature patterns that will be removed").action(() => {
  ...
  patterns.forEach((p) => logger2.success(p));
  logger2.blank();
  logger2.info(`${patterns.length} pattern groups will be removed from commits`);
});
program2.command("setup-author").description("Change git author if current author is an AI").action(async () => {
  ...
  if (currentName.exists && currentName.value) {
    if (isAIAuthor(currentName.value)) {
      logger2.warning(`Current author: ${logger2.yellowText(currentName.value)} ${logger2.dim("(AI detected)")}`);
    } else {
      logger2.info(`Current author: ${logger2.cyan(currentName.value)}`);
    }
    if (currentEmail.exists && currentEmail.value) {
      logger2.info(`Current email:  ${logger2.cyan(currentEmail.value)}`);
    }
    logger2.blank();
  }
  const nameInit = currentName.exists && currentName.value && !isAIAuthor(currentName.value) ? currentName.value : "";
